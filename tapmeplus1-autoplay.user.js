// ==UserScript==
// @name         TapMePlus1 自动通关 (优化版)
// @namespace    http://tampermonkey.net/
// @version      3.6
// @description  自动通关脚本，专注于连锁反应和高效得分 (优化版)
// @author       泡小胡呢
...
(function () {
    ...
    const defaultConfig = {
        ...
        animationWaitTimeout: MAX_ANIMATION_WAIT_TIME, // 动画等待超时时间
        clickDelay: BASE_CONTINUOUS_CLICK_DELAY, // 点击延迟
        randomClickProbability: 0.05 // 随机点击概率
    };
    ...
    let controlPanel, statusDiv, logArea, startBtn, pauseBtn, resetBtn, exportBtn, configArea;
    ...
    // 动画等待
    async function waitForAnimationToFinish(timeout = config.animationWaitTimeout, interval = ANIMATION_WAIT_INTERVAL) {
        let start = Date.now();
        let lastKnownBoard = getBoardFromDOM();
        let animating = isGameAnimating(); // 初始状态
        ...
        while (Date.now() - start < timeout) {
            ...
            // 检查动画状态和棋盘状态
            const currentAnimating = isGameAnimating();
            const currentBoardCheck = getBoardFromDOM();

            if (currentAnimating !== animating) {
                animating = currentAnimating; // 更新动画状态
                if (animating) {
                    start = Date.now(); // 动画重新开始，重置计时器
                    log('[动画等待] 动画状态变化，重新开始等待...');
                }
            }
    ...
    function findBestClick(currentBoard, clicksLeft, score) {
        ...
        // 随机点击
        if (!bestPrediction && Math.random() < config.randomClickProbability) {
            log('尝试随机点击...');
            const availableCells = [];
            for (let r = 0; r < BOARD_SIZE; r++) {
                for (let c = 0; c < BOARD_SIZE; c++) {
            ...
            if (availableCells.length > 0) {
                const randomIndex = Math.floor(Math.random() * availableCells.length);
                const randomCell = availableCells[randomIndex];
                const randomClickTimes = Math.floor(Math.random() * Math.min(clicksLeft, config.maxContinuousClicks)) + 1;
                bestPrediction = predictClick(randomCell.r, randomCell.c, randomClickTimes);
                if (bestPrediction) {
                    log(`随机点击 (${bestPrediction.row},${bestPrediction.col}) x${bestPrediction.clickTimes}`);
                }
            }
        }
    ...
    async function autoClickLoop() {
        ...
        while (running && !stopRequested) {
            ...
            if (clicksLeft <= 0) {
                ...
                await waitForAnimationToFinish(config.animationWaitTimeout * 2); // 等待可能发生的连锁反应
            ...
            // 执行点击
            let delay = config.clickDelay;
            if (clicksLeft <= 2) delay = MIN_CONTINUOUS_CLICK_DELAY;
            else if (clicksLeft <= 4) delay = config.clickDelay / 2; // 对4点及以下调整
    ...
    function createControlPanel() {
        ...
        title.textContent = 'TapMePlus1 自动通关版 (v3.6)';
        ...
        configHeader.textContent = '配置 (点击折叠/展开)';
        configHeader.style.cursor = 'pointer';
        configHeader.style.userSelect = 'none';
        configHeader.style.marginBottom = '4px';
        configHeader.style.fontWeight = 'bold';
        controlPanel.appendChild(configHeader);

        configArea = document.createElement('div');
        configArea.id = 'autoTapmeConfigArea';
        Object.assign(configArea.style, {
            backgroundColor: '#1c1c1c', border: '1px solid #444', overflowY: 'auto',
            padding: '8px', whiteSpace: 'pre-wrap', userSelect: 'text', fontSize: '13px',
            lineHeight: '1.4em', borderRadius: '4px', display: 'none' // 初始隐藏
        });
        controlPanel.appendChild(configArea);

        configHeader.onclick = () => {
            if (configArea.style.display === 'none') {
                configArea.style.display = 'block';
                renderConfig(); // 展开时渲染配置
            } else {
